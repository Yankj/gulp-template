...
gulp.task('sceInit', function() {
	...
	generateFile("nginx_server.inc", result)
		.pipe(gulp.dest(config.dist + 'sce/conf'));
	...
	var uwsgiTpl = `
		...
		gid=app
	`;
	generateFile("uwsgi.ini", uwsgiTpl)
		.pipe(gulp.dest(config.dist + 'sce/conf'));
...
start: $UWSGI_START`,
	...
	return generateFile("app.yaml", lodash.template(yamlTpl)(obj))
